...
- To keep a whole **organization's** seats out of auto-switch — its admin disabled Claude Code access, so every request on those seats fails while their usage still reads as plenty of room — run `cswap block-org <org> --evidence "<the error line, or why>"`. `<org>` is the label `cswap list` prints in brackets (`[Acme]`), matched exactly; it is never the email domain, and a label no managed account shows is refused. From the next tick `cswap auto` skips those seats for every trigger (`skip: org blocked <org> (#6, #7)` in its log, every tick) and, if the *active* seat is one of them, fails over off it immediately. **A block never expires by itself** — there is no TTL. It is lifted by `cswap unblock-org <org>`, or by a watchdog that wrote it (`--by watchdog`), which must give its resolved evidence and can clear only its own entries. `cswap blocked-orgs` shows who blocked what, when and why; `cswap list` marks the seats `(org blocked)` and `cswap status` says so for the active one. Blocking is a ranking filter only: it never disables, enables or switches a seat, and a blocked seat stays a valid explicit `cswap switch <num|email>` target. The record is `blocked-orgs.json` in the backup root. It fails safe: if it is missing or unparseable, nothing is excluded and `cswap auto` prints a `WARNING: blocked-orgs record …` line on every tick rather than crashing or staying quiet — run `cswap blocked-orgs --init` once so that a record going missing later means something. If every seat that could otherwise take over is blocked, auto-switch stays where it is and logs `ALARM: all eligible seats blocked` each tick instead of thrashing.
...
cswap block-org Acme --evidence "org disabled Claude Code"   # Keep an org's seats out of auto-switch (no expiry)
cswap unblock-org Acme       # Lift it
cswap blocked-orgs              # Show blocked orgs: who, when, why (--json, --init)
